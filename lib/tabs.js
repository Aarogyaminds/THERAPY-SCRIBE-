...
export async function processAllTabs(patientId, transcript, sessionDate) {
  ...
  const { data: tabsRow } = await db.from("tabs").select("*").eq("patient_id", patientId).single();
  ...
  const [briefResult, recapResult, patternsResult] = await Promise.allSettled([
  ...
  const { data: entities } = await db.from("life_context_entities").select("id, name, type").eq("patient_id", patientId);
  try {
    const lcResult = await callGeminiJSON(buildLifeContextPrompt(entities || [], transcript, sessionDate));
  ...
  const { data: openThreads } = await db
    ...
    .eq("status", "open");
  try {
    const otResult = await callGeminiJSON(
      buildOpenThreadsPrompt((openThreads || []).map((t) => ({ id: t.id, description: t.description })), transcript, sessionDate)
    );
    ...
    for (const nt of otResult?.new_threads || []) {
